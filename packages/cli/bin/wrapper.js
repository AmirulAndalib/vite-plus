...
function getPackageName() {
  ...
  return `@voidzero-dev/vite-plus-${platform}-${arch}${suffix}`;
}

function getBinaryPath() {
  const binaryName = process.platform === 'win32' ? 'vp.exe' : 'vp';

  // 1. First check for local binary in vp-binary/ directory (local development)
  const localBinaryPath = join(__dirname, '..', 'vp-binary', binaryName);
