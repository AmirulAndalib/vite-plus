...
fn get_package_graph(workspace_root: impl AsRef<Path>) -> anyhow::Result<Graph<PackageInfo, ()>> {
    ...
    for member in member_globs.get_package_json_paths(workspace_root)? {
        let package_json: PackageJson = serde_json::from_slice(&fs::read(&member)?)?;
        let path = member.strip_prefix(workspace_root).with_context(|| {
            format!(
        ...
        let path = path
            .to_str()
            .with_context(|| format!("Package path {:?} is not valid UTF-8", path))?;
        graph_builder.add_package(path.into(), package_json)?;
    }
    Ok(graph_builder.build())
}

fn main() -> anyhow::Result<()> {
    let package_graph = get_package_graph(current_dir()?)?;
